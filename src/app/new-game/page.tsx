    ...
    try {
      ...
      if (response.ok) {
        ...
        if (socketRef.current && socketRef.current.connected) {
          ...
          // Fallback se socket não estiver disponível
          setPlacedBet({
            amount: amountToUse,
            timestamp: Date.now()
          });
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card variant="bordered" className="md:col-span-2 border border-gray-800 bg-gradient-to-b from-[#121212] to-[#0c0c0c] shadow-xl overflow-hidden h-full">
          ...
          <CardContent className="p-0">
            ...
            <div className="bg-[#0a0a0a] border-t border-gray-800/50 rounded-b-lg p-4">
                ...
                <div className="animate-fadeInUp">
                  <div className="mb-4">
                    ...
                    <div className="grid grid-cols-5 gap-2 quick-bet-buttons">
                      {customQuickBets.map((bet, index) => (
                        <Tooltip
                          key={bet}
                          content={`Apostar R$ ${bet.toFixed(2)}`}
                          position="bottom"
                          className={tooltipsEnabled ? '' : 'hidden'}
                        >
                          <button
                      ...
                      {/* Input personalizado */}
                      <div className="col-span-5 mt-3 animate-fadeIn delay-500">
                        <Input
                          type="number"
              ...
              {/* Área de CashOut */}
              {placedBet && currentPhase === 'running' && !cashedOut && (
                <div className="bg-[#121212] rounded-lg p-4 border border-gray-800/80 shadow-lg animate-fadeInUp">
                      ...
                      >
                        <div>
                          ...
                          <div className={`font-medium text-lg ${getMultiplierColor(currentMultiplier)}`}>
                            R$ {(placedBet.amount * (typeof currentMultiplier === 'number' && !isNaN(currentMultiplier) ? currentMultiplier : 1.0)).toFixed(2)}
                          </div>
                        </div>
                      </Tooltip>
                    </div>
                  ...
                  <Tooltip
                    content={`Clique para garantir seus ganhos atuais: R$ ${(placedBet.amount * (typeof currentMultiplier === 'number' && !isNaN(currentMultiplier) ? currentMultiplier : 1.0)).toFixed(2)}`}
                    position="top"
                    className={tooltipsEnabled ? '' : 'hidden'}
                  >
                    <Button 
                      ...
                      disabled={isLoading}
                    >
                      {isLoading ? 'Processando...' : `CASH OUT ${(placedBet.amount * (typeof currentMultiplier === 'number' && !isNaN(currentMultiplier) ? currentMultiplier : 1.0)).toFixed(2)}`}
                    </Button>
                  </Tooltip>
                </div>
              )}
              ...
              {placedBet && currentPhase === 'betting' && (
                <div className="bg-[#121212] rounded-lg p-4 border border-gray-800/80 text-center shadow-lg animate-scaleIn">
              ...
              {/* CashOut feito */}
              {cashedOut && cashOutMultiplier && (
                <div className="bg-[#121212] rounded-lg p-4 border border-green-500/20 text-center shadow-lg shadow-green-500/5 animate-scaleIn">
                  ...
                  <div className="text-2xl font-bold text-white animate-fadeIn delay-100 animate-shimmer">
                    R$ {((placedBet?.amount || 0) * cashOutMultiplier).toFixed(2)}
                  </div>
                  <div className="text-sm text-gray-400 mt-2 animate-fadeIn delay-200">
        ...
        <div className="space-y-6">
          ...
          <AutoBetConfig
            ...
            isActive={isAutoBetting}
            onStart={startAutoBetting}
            onStop={stopAutoBetting}
            onAutoModeChange={handleAutoModeChange}
            minBet={MIN_BET_AMOUNT}
            maxBet={Math.min(MAX_BET_AMOUNT, userBalance)}
            isGamePhase={currentPhase !== 'betting'}
          ...
          <Card variant="bordered" className="border border-gray-800 bg-[#0f0f0f] shadow-lg">
            <CardHeader className="p-4 border-b border-gray-800/40">
              ...
              <CardDescription>Regras e instruções</CardDescription>
            </CardHeader>
            <CardContent className="p-4">
              <div className="space-y-3 text-sm text-gray-400">
                <p>
                  <strong className="text-white">O Multiplicador</strong> é um jogo onde você deve fazer Cash Out no momento certo para maximizar seus ganhos.
                </p>
                <p>
                  1. Faça sua aposta durante a fase de apostas (5 segundos).
                </p>
                <p>
                  2. O multiplicador começa em 1.0x e varia aleatoriamente entre 0.0x e 2.0x.
                </p>
                <p>
                  3. Clique em &quot;Cash Out&quot; para garantir seu ganho atual.
                </p>
                <p>
                  4. Seus ganhos = valor apostado × multiplicador no momento do Cash Out.
                </p>
                <p>
                  5. Se não fizer Cash Out, seu resultado será baseado no multiplicador final.
                </p>
                <p>
                  6. Se o multiplicador for menor que 1.0x no final, você perderá parte da sua aposta.
                </p>
              </div>
            </CardContent>
          </Card>
          
          {/* Jogadores Online e Apostas Atuais */}
          <Card variant="bordered" className="border border-gray-800 bg-[#0f0f0f] shadow-lg">
            <CardHeader className="p-4 border-b border-gray-800/40">
              ...
              <CardDescription>
                <span className="flex items-center">
                  ...
                  <span className="text-gray-400">{playerCount} jogadores conectados</span>
                </span>
              </CardDescription>
            </CardHeader>
            <CardContent className="p-4">
              {allBets.length > 0 ? (
                <div className="space-y-2 max-h-[200px] overflow-y-auto pr-2 scrollbar-hide">
                  {allBets.map((bet, index) => (
                    <div key={index} className="flex justify-between items-center bg-gray-800/30 p-2 rounded-md border border-gray-800">
                      <div className="flex items-center">
                        <div className="w-8 h-8 bg-gradient-to-br from-indigo-500 to-purple-500 rounded-full flex items-center justify-center text-xs font-bold">
                          {bet.playerId.substring(0, 2)}
                        </div>
                        <div className="ml-2">
                          <div className="text-sm font-medium">
                            {bet.playerId === session?.user?.id ? 'Você' : `Jogador ${index + 1}`}
                          </div>
                      ...
                      <div className="text-right">
                        <div className="font-bold text-[#3bc37a]">R$ {bet.amount.toFixed(2)}</div>
                      </div>
                    </div>
                  ))}
                </div>
